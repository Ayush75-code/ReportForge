...
*   **SUCCESS** = Generate a structured analysis that:
    1.  Calculates a **Performance Score** (0-100) that answers "Is it good or bad?"
    2.  Generates a **Quick Summary** (single scannable sentence)
    3.  Identifies the single most important insight (good or bad)
    4.  Highlights exactly 3 wins with supporting data
    5.  Flags exactly 2 concerns that need attention
    6.  Provides exactly 3 specific, actionable recommendations
...
*   **Base Score:** 50
*   **+20:** If avg CPA is below industry benchmark ($30 for most verticals)
*   **+15:** If CTR > 2%
*   **+10:** If no campaigns with 0 conversions
*   **+5:** If conversion volume increased vs previous period (if data available)
*   **-10:** For each critical concern identified
*   **-5:** For each campaign with CTR < 1%
*   **Clamped:** 0-100 range

**Score Labels:**
*   80-100: "Strong Performance"
*   50-79: "Moderate Performance"
...
```json
{
  ...
  "agency": {
    "name": "string",
    "logo_url": "string (optional, uses text fallback if not set)",
    "brand_color": "hex string (default: #6366f1)"
  },
  "client": {
    "name": "string"
  },
  "analysis": {
    ...
    "score_label": "string (Strong/Moderate/Needs Attention)",
    "quick_summary": "string (single sentence)",
    "summary": "string",
    "wins": [],
    "concerns": [],
    "recommendations": []
  },
  "personal_note": "string (optional, user-provided human touch)",
  "metadata": {
    "date_range": "string",
...
*   **Font:** Helvetica.
*   **Header:** Logo/Name (Left) + Title/Client (Right) + **Performance Score Badge** (Top Right).
*   **Quick Summary:** Highlighted box below header with single-sentence overview.
*   **Sections:** Summary (Box) -> Wins (Checkmarks) -> Concerns (Triangles) -> Recommendations (Numbers).
*   **Personal Note (if provided):** Italicized block after Recommendations, labeled "Note from your account team".
